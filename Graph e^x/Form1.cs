...
namespace proba
{
    public partial class Form1 : Form
    {
        ...
        int poz_x = 50;
        int poz_y = 50;
        int Valoare_maxima_x = 250;
        int Inaltime_chenar = 250; //Inaltime chenar/50 de diviziuni ==> patru patrate
        int Valoare_maxima_y = 100;
        ...
        {
            int Amplificarea = 50;
            double x = 0; //Valoare inceput
            double d_max = 6; //La valori mai mari da eroare 
            double pas = d_max / Valoare_maxima_x;
            for (int i = 1; i <= Valoare_maxima_x; i++)
            {
               
                int f = System.Convert.ToInt32(Amplificarea * (Math.Pow(Math.E, x)));
                if (f > Valoare_maxima_x)
                {
                    x = 0;
                }
                x += pas;
